...
robust_FPCA_Bali_Bspline = function(data, harmonics_tgt, gaitbasis, num_comp, N){
  ...
  #data = Y
  #num_comp = 3
  #gaittime <- seq(0,1,len=N) #Dongjiao comment this
  gaittime <- seq(0,1,len=ncol(data))#Dongjiao add this ### Dongjiao comment this
  #gaitrange <- c(0,1) ### Dongjiao comment this
  #nord=50 #Dongjiao comment this
  #nord=10 #Dongjiao add this ### Dongjiao comment this
  #gaitbasis <- create.bspline.basis(c(0,1), nbasis = nord, norder = 3) #Dongjiao comment this
  #gaitbasis <- create.bspline.basis(c(0,1), nbasis = 20, norder = 4) #Dongjiao add this ### Dongjiao comment this
...
#Dongjiao add one additional input to enable the spline order and numbers can be tuned.
#The original function is BIC_fit_bali_trim = function(Sim_output, rob_comp_X,rob_comp_Y, breakdown_p)
BIC_fit_bali_trim = function(Sim_output, rob_comp_X,rob_comp_Y, breakdown_p, gaitbasis){
  ...
  set.seed(123)
  #gaittime <- seq(0,1,len=N) #Dongjiao comment this
  gaittime.1 <- seq(0,1,len=N) #Dongjiao add this
  gaittime.2 <- seq(0,1,len=ncol(Y)) #Dongjiao add this
  gaitrange <- c(0,1)
  nord=200
  #gaitbasis <- create.bspline.basis(c(0,1), nbasis = nord, norder = 3) #Dongjiao comment this
  ## gaitbasis <- create.bspline.basis(c(0,1), nbasis = 20, norder = 4) #Dongjiao add this ### Dongjiao comment this

  #oneWipe = array( dim = c(N,nsamples,2) ) #Dongjiao comment this

  #oneWipe[,,1] = t(X.all) #Dongjiao comment this


  #oneWipe[,,2] = t(Y) #Dongjiao comment this

  fdParobj = fdPar(gaitbasis,int2Lfd(2),lambda=0) #Applies smoothness to the functions
  #gaitfd <- smooth.basisPar(gaittime, oneWipe, gaitbasis, Lfdobj=NULL, lambda=0)$fd #Dongjiao comment this
  gaitfd.1 <-smooth.basisPar(gaittime.1, t(X.all), gaitbasis, Lfdobj=NULL, lambda=0)$fd #Dongjiao add this
  gaitfd.2 <-smooth.basisPar(gaittime.2, t(Y), gaitbasis, Lfdobj=NULL, lambda=0)$fd #Dongjiao add this

  # FPCA on Y
  #resp_fd<- gaitfd[,2] #Dongjiao comment this
  resp_fd<- gaitfd.2 #Dongjiao add this
  ...
  # FPCA on X
  #pcs = pca.fd(gaitfd[,1],nharm=rob_comp_X,fdParobj) #Dongjiao comment this
  pcs = pca.fd(gaitfd.1,nharm=rob_comp_X,fdParobj) #Dongjiao add this
  harmonics_x = pcs$harmonics
  pred_scores=pcs$scores
  #print(sum(pcs$varprop[1:comp_X]))

  ### Robust FPCA
  #robY = robust_FPCA_Bali(Y, harmonics_tgt, gaitbasis, rob_comp_Y, N) #Dongjiao comment this
  robY = robust_FPCA_Bali(Y, harmonics_tgt, gaitbasis, rob_comp_Y, ncol(Y)) #Dongjiao add this
  ...
  #beta_est  = MLTS_iterated_trim(rob_coefX, rob_coefY,nsamples,breakdown_p)$beta #Dongjiao comment this the functio MLTS_iterated_trim does not exist
  beta_est  = MLTS_iterated_reweighted(rob_coefX, rob_coefY,nsamples)$beta #Dongjiao add this

  #Y_hat_rob = matrix(ncol= N, nrow = nsamples)#Dongjiao comment this
  Y_hat_rob = matrix(ncol= ncol(Y), nrow = nsamples)#Dongjiao add this
  PE = rep(0,nsamples)
  #rob_residual = matrix(ncol=nsamples, nrow=N) #Dongjiao comment this
  rob_residual = matrix(ncol=nsamples, nrow=ncol(Y)) #Dongjiao add this
  for (dd in 1:nsamples){
    Xtest <- rob_coefX[dd,]
    truth=Y[dd,]
    rec_one_out= Pred_MFLR(Xtest, beta_est, robust_FPCA_y) + med_mu_y

    #Y_hat_rob[dd,] = eval.fd(rec_one_out, gaittime) #Dongjiao comment this
    Y_hat_rob[dd,] = eval.fd(rec_one_out, gaittime.2) #Dongjiao add this
    PE[dd] = inprod(resp_fd[dd,]-rec_one_out,resp_fd[dd,]-rec_one_out)
    #rob_residual[,dd] = truth-eval.fd(rec_one_out, gaittime) #Dongjiao comment this
    rob_residual[,dd] = truth-eval.fd(rec_one_out, gaittime.2) #Dongjiao add this
  }
...
##Dongjiao added another input gaitbasis to the input ##
##The original function is class_BIC_fit = function(Sim_output, rob_comp_X,rob_comp_Y)
class_BIC_fit = function(Sim_output, rob_comp_X,rob_comp_Y, gaitbasis ){
  ...
  X.all = Sim_output$Predictor #Dongjiao add this
  Y = Sim_output$Response #Dongjiao add this

  #gaittime <- seq(0,1,len=N) #Dongjiao comment this
  gaittime.1 <- seq(0,1,len=ncol(X.all)) #Dongjiao add this
  gaittime.2 <- seq(0,1,len=ncol(Y)) #Dongjiao add this

  gaitrange <- c(0,1)
  nord=200
  #gaitbasis <- create.bspline.basis(c(0,1), nbasis = nord, norder = 3) #Dongjiao comment this
  #gaitbasis <- create.bspline.basis(c(0,1), nbasis = 20, norder = 4) #Dongjiao add this##Dongjia comment this
  ...
  #Y_hat = matrix(ncol= N, nrow = nsamples) # estimates  #Dongjiao comment this
  #classical_residuals = matrix(ncol=nsamples, nrow=N) #residuals #Dongjiao comment this
  Y_hat = matrix(ncol= ncol(Y), nrow = nsamples) # estimates  #Dongjiao add this
  classical_residuals = matrix(ncol=nsamples, nrow=ncol(Y)) #residuals #Dongjiao add this

  for (dd in 1:nsamples){
    Xtest <- coefX[dd,]
    truth= Y[dd,]
    rec_one_out= Pred_MFLR(Xtest, classical_beta, harmonics_tgt) + mu_y
    #Y_hat[dd,] = eval.fd(rec_one_out, gaittime) #Dongjiao comment this

    #classical_residuals[,dd] = truth-eval.fd(rec_one_out, gaittime) #Dongjiao comment this

    Y_hat[dd,] = eval.fd(rec_one_out, gaittime.2) #Dongjiao add this

    classical_residuals[,dd] = truth-eval.fd(rec_one_out, gaittime.2) #Dongjiao add this
  }
...
### Dongjiao change the input to the function, gaitbasis was added to enable the spline order and numbers can be tuned
## The original function should be rob_depth = function(Sim_output, rob_comp_X,rob_comp_Y)
rob_depth = function(Sim_output, rob_comp_X,rob_comp_Y, gaitbasis){
...
#Dongjiao add gaitbasis as the input to tune the number of basis and the orther of the splines.
## The original function is class_depth = function(Sim_output, comp_X, comp_Y)
class_depth = function(Sim_output, comp_X, comp_Y,gaitbasis){
 ...
 # oneWipe[,,2] = t(Y) #Dongjiao comment this
  ...
  # FPCA on X
  #pcs = pca.fd(gaitfd[,1],nharm=comp_X,fdParobj) #Dongjiao comment this
  pcs = pca.fd(gaitfd.1,nharm=comp_X,fdParobj) #Dongjiao add this
  harmonics_x = pcs$harmonics
  pred_scores=pcs$scores
  #print(sum(pcs$varprop[1:comp_X]))
