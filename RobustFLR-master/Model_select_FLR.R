...
#### n.basis and n.order are two variables Dongjiao added to the original code. 
#### The original piece only have Model_select_FLR = function(Sim_output, max_number)
Model_select_FLR = function(Sim_output, max_number, n.basis, n.order){
  ...
  for(hhh in 2:(max_number+1)){
    for (ggg in 2:(max_number+1)){
      ...
      gaitbasis <- create.bspline.basis(c(0,1), nbasis = n.basis, norder = n.order) #Dongjiao add this
      ...
      # FPCA on X
      #pcs = pca.fd(gaitfd[,1],nharm=num_comp_X,fdParobj) #Dongjiao comment this
      pcs = pca.fd(gaitfd.1,nharm=num_comp_X,fdParobj) #Dongjiao add this
  ...
  RBIC_rob_val = BIC_fit_bali_trim(Sim_output, RBIC_comp_X,RBIC_comp_Y,breakdown_p, gaitbasis)
  ####### Dongjiao add this ################
  Data.train<-list(N=N, nsamples=100, num_out=50, 
                   Predictor=rbind(X.all[1:50,], X.all[1:50,]), Response=rbind(Y[1:50,], Y[1:50,]))#Dongjiao add this for training accur
  RBIC_rob_val_train = BIC_fit_bali_trim(Data.train, RBIC_comp_X,RBIC_comp_Y,breakdown_p, gaitbasis) #Dongjiao add this for training accuracy
  ###########################
  BIC_rob_val = BIC_fit_bali_trim(Sim_output, BIC_comp_X,BIC_comp_Y,breakdown_p, gaitbasis)

  BIC_val = class_BIC_fit(Sim_output, comp_X,comp_Y, gaitbasis)
#############################
############Dongjiao add RBIC_rob_val_train to the output. 
...
return(Models = list(RBIC_comp_X = RBIC_comp_X, RBIC_comp_Y = RBIC_comp_Y, BIC_comp_X = BIC_comp_X, BIC_comp_Y = BIC_comp_Y, comp_X = comp_X, comp_Y = comp_Y,
                     RBIC_val = RBIC_rob_val, BIC_val = BIC_rob_val, BIC_class_val = BIC_val,  RBIC_val_train = RBIC_rob_val_train) )
