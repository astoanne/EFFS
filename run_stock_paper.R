...
setwd(project_root)
source(main_file)
## ====== 1) è¯»å–ä¸Žæˆå½¢çœŸå®žæ•°æ®ï¼ˆSINGLE INPUT, SINGLE OUTPUTï¼‰ ======
t_data <- read.csv(file.path(data_dir, "stock.csv"), header = FALSE)
t_data <- t_data[2:nrow(t_data), ]
len_data <- nrow(t_data)

# æŠŠä¸€åˆ—æ—¶é—´åºåˆ— reshape æˆ â€œæ ·æœ¬ Ã— ç½‘æ ¼ç‚¹æ•°â€ çš„çŸ©é˜µï¼Œå¹¶åš [0,1] å½’ä¸€åŒ–
get_format_matrix <- function(vec, n_rows = 22, n_cols = 457) {
  ...
  t(mat)  # (æ ·æœ¬æ•° = n_cols) Ã— (ç½‘æ ¼ç‚¹æ•° = n_rows)
}

# -------- Choose ONE input and ONE output --------
input_name  <- "AAPL"    # <-- CHANGE HERE if needed
output_name <- "NVDA"
...
cat("Running SISO stock experiment:",
...
## =========================
## 4b) Plot rule-number trajectories (reusing original base Â·Â·Â·ting)
##      â€” and save PNGs automatically
## =========================
...
if (!is.null(rm_none)) {
  ...
  plot_rule_traj_base(
    ...
    main_text = expression(
      alpha[add] == 0.5 ~ "," ~
        tau[merge] == 0.9),
    file_path = file.path(output_dir, "noprune_stock.png"),
    ylim_range = y_range
  )
}
...
# if (!is.null(rm_ema)) {
#   # x2 <- seq_along(rm_ema)
#   x2 <- seq(0, 1, length.out = length(rm_ema))
#   plot_rule_traj_base(
#     x = x2, y = rm_ema,
#     main_text = paste0("Îµ_a = ", add.thr, ", Îµ_m = ", merge.thr, " (EMA pruning)"),
#     file_path = file.path("prune_stock_old.png"),
#     ylim_range = y_range
#   )
# }
if (!is.null(rm_ema)) {
  ...
  plot_rule_traj_base(
    ...
    file_path = file.path(output_dir, "prune_stock.png"),
    ylim_range = y_range
  )
}
...
# 5.1 Interaction-PLSï¼ˆBeyaztas & Shangï¼‰
# 5.1 Interaction-PLSï¼ˆBeyaztas & Shangï¼‰
# 5.1 Interaction-PLSï¼ˆBeyaztas & Shangï¼‰
...
run_ovglasso <- function() {
  ...
  if (exists("ncol.in.each")) {
    ...
    # é€‰ä¸€ä¸ªä¸Ž r æŽ¥è¿‘çš„ Lï¼ˆåªæ˜¯å…œåº•ï¼Œä¸å»ºè®®èµ°åˆ°è¿™é‡Œï¼‰
...
csv_name  <- file.path(output_dir, "stock_paper_results.csv")
