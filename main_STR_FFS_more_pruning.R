...
Dist.L2<-function(Xa, Time.t, Xb){
  #Compute the L^2 distance betweeen curves Xa and Xb.
  ##Remark.  The functional data form of Xa and Xb are obtained by spline fitting. 
  #If Xa is not in a functional data form (coeffs and basis), then assume that the values of Xa on time stamps 
  #  t_{1}, t_{2}, \ldots, t_{L_{x}} (Time.t) are known.
  ########################################################################
  ##Input:
  #Time.t --- Time stamps to compute the integral, or time stamps on which the value of Xa or Xb are given.
  #           If both Xa and Xb are in the functional form, then it is better to refine the diviation Time.t.
  #Xa --- It could be a vector with given values on a certain curve, or a curve presented in the functional form (coeffs and basis). 
  #Xb --- See above.
  
  ##Output:
  #L2 --- It is the square of L2 distance between curve Xa and Xb.
  
  ######################################################################
  ##Check whether Xa and Xb are in the functional form.
  Isfd.a<-is.fd(Xa)
  Isfd.b<-is.fd(Xb)
  #######################################################################
  ##Calculate the L^2 distance between Xa and Xb over different cases.
  ...
  if ((Isfd.a==0)&(Isfd.b==0)){
    #Both Xa and Xb are given values of the curves.
    ...
    #Xa gives a serise of values on a certain curve.
    #Xb is in functional data form.
    ...
    #Xa is in functional data form.
    #Xb gives a serise of values on a certain curve.
    ...
    #Both Xa and Xb are in functional data form.
    Xa.value<-eval.fd(Time.t, Xa)
    Xb.value<-eval.fd(Time.t, Xb)
...
#################################
#membership.fun<-function(coeffs , radius, cc.fd, x.spline, Time.t){
#  cc.fd$coefs<-coeffs
#  membership<-exp(-Dist.L2(x.spline$X.fd, Time.t, cc.fd)/(2*(radius^2)))
#  return(membership)
#}
...
main.STR.FFS.visual <- function(add.thr, merge.thr, Time.t, Time.t.len,
  ...
  ## (The rest of your existing setup remains unchanged up to first clustering step) ##
  ...
  util_avg[1]    <- 0
  act_ema[1] <- 0
  n.clu.max.snapshot <- n.clu
  max.rule.time <- 1
  center.max.snapshot <- center[seq_len(n.clu), , drop = FALSE]
  cons.para.max.snapshot <- cons.para[seq_len(n.clu), , drop = FALSE]
  capture_rule_snapshot <- function(time_step) {
    if (n.clu > n.clu.max.snapshot) {
      n.clu.max.snapshot <<- n.clu
      max.rule.time <<- time_step
      center.max.snapshot <<- center[seq_len(n.clu), , drop = FALSE]
      cons.para.max.snapshot <<- cons.para[seq_len(n.clu), , drop = FALSE]
    }
    invisible(NULL)
  ...
  #PN<-list(diag(nbasis.output))
  #Store(PN)
  #PN<-1 # PN is used to update the consequent parameters 
  #PN needs to be udated in every evolving step.
  #####################################################
  #\{\int B(t)B(t)^{T} dt\}^{-1} only needs to be calculated for one timw.
  ...
  util_avg[n.clu]  <- 0
  act_ema[n.clu] <- 0
  capture_rule_snapshot(t)
  
  #PN<-rbind(PN, list(diag(nbasis.output)))
  #Store(PN)
  #gc()
  ...
  #B.Bprime<-matrix(list(), 1, (Time.output.len-1))
  #for (i in 1:(Time.output.len-1)){
  #  B.Bprime[[i]]<-((matrix(B.t[i+1,], nbasis.output, 1)%*%(y.data[i+1]-matrix(B.t[i+1,], 1, nbasis.output)%*%matrix(cons.para[1,], nbasis.output, 1))
  #                   +matrix(B.t[i,], nbasis.output, 1)%*%(y.data[i]-matrix(B.t[i,], 1, nbasis.output)%*%matrix(cons.para[1,], nbasis.output, 1)))*(Time.output[i+1]-Time.input[i])/2)
  #}
  #B.Bprime<-Reduce('+', B.Bprime)
  ...
  ####
  #Update the consequent parameters using the fitted output
  #B.t<-eval.basis(Time.output, B.spline.Y)
  ...
  #PN[1]<-PN[1]-(PN[1]*membership[1]*PN[1])/(1+membership[1]*PN[1])
  #cons.para[1,]<-cons.para[1,]+(PN[1]*B.prime*membership[1])%*%B.Bprime
  ####
  ...
  while(null.x==0){
      ...
      if (n.clu>1){
        ...
        if (max(sim)>merge.thr){
          ...
          radius[max.act]<-sqrt( (1/(c.num[max.act]+c.num[max.sim])) * ( c.num[max.act] * radius[max.act]^2 + c.num[max.sim] * radius[max.sim]^2 
                                                                         + c.num[max.act] * Dist.L2(cen.a, Time.t, cen.c) 
          ...
          # EMA isnâ€™t strictly additive; a reasonable choice is importance-weighted average
          ...
          #remove(center.tem, cons.para.tem, radius.tem, c.num.tem, PN.tem, center.new, max.sim, max.act)
          gc()
        }
        #rm(sim)
        gc()
    ...
    #if (n.clu==1){
    #  center[1,]<-matrix(center[1, ], nrow = 1)
    #  cons.para[1,]<-matrix(cons.para[1, ], nrow = 1)
    ...
    for (k in 1:n.clu){
      #B.Bprime<-matrix(list(), 1, (Time.output.len-1))
      # for (i in 1:(Time.output.len-1)){
      #  B.Bprime[[i]]<-((matrix(B.t[i+1,], nbasis.output, 1)%*%(y.data[i+1]-matrix(B.t[i+1,], 1, nbasis.output)%*%matrix(cons.para[k,], nbasis.output, 1))
      #                    +matrix(B.t[i,], nbasis.output, 1)%*%(y.data[i]-matrix(B.t[i,], 1, nbasis.output)%*%matrix(cons.para[k,], nbasis.output, 1)))*(Time.output[i+1]-Time.input[i])/2)
      # }
      # B.Bprime<-Reduce('+', B.Bprime)
      # PN[[k]]<-PN[[k]]-(membership[k]*PN[[k]]%*%B.prime)%*%inv(membership[k]*PN[[k]]%*%B.prime+diag(nbasis.output))%*%PN[[k]]
      # cons.para[k,]<-cons.para[k,]+membership[k]*PN[[k]]%*%B.Bprime
      PN[[k]]<-PN[[k]]-(membership[k]*PN[[k]]%*%B.prime)%*%inv(membership[k]*PN[[k]]%*%B.prime+diag(nbasis.output))%*%PN[[k]]
      
      cons.para[k,]<-matrix(cons.para[k,],nrow=1)+membership[k]*(matrix(y.spline$coeff.X, nrow = 1)-matrix(cons.para[k,],nrow=1))%*%PN[[k]]%*%B.prime
      
      #PN[k]<-PN[k]-(PN[k]*membership[k]*PN[k])/(1+membership[k]*PN[k])
      #cons.para[k,]<-cons.para[k,]+(PN[k]*B.prime*membership[k])%*%B.Bprime
  ...
  results<-list(y.train.func.mse=y.train.func.mse, y.test.func.mse=y.test.func.mse,
                ...
                n.clu.max.snapshot=n.clu.max.snapshot, max.rule.time=max.rule.time,
                center.max.snapshot=center.max.snapshot,
                cons.para.max.snapshot=cons.para.max.snapshot)
  return(results)
  
  #return(list(y.train.func.mse=y.train.func.mse, y.test.func.mse=y.test.func.mse,
  #       y.train.real.mse=y.train.real.mse, y.test.real.mse=y.test.real.mse,
  #       y.train.func.mse.c=y.train.func.mse.c, y.test.func.mse.c=y.test.func.mse.c,
  #       y.train.real.mse.c=y.train.real.mse.c, y.test.real.mse.c=y.test.real.mse.c,
  #       n.rule=nrow(center), rule.ave=rule.ave, rule.max=rule.max ))
}
