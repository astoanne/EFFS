...
## ====== 1) Reading & Shaping (Unified Scale, SINGLE INPUT) ======
t_data <- read.csv('real_data/north_dakota_agricultural.csv', header = FALSE)
...
cat("Running agricultural experiment:", input_name, "->", output_name, "\n")

## ====== 2) Parallelization (Only for FFS) ======
n_cores <- max(1, detectCores()-1)
cl <- makeCluster(n_cores)
registerDoParallel(cl)
clusterEvalQ(cl, { source("main_effs.R") })

## ====== 3) Unified Evaluation Tools ======
...
# Store the rule.monitor trajectory for each pruning strategy
...
run_ffs_once <- function(ps) {
  ...
  ## Save the evolution of the number of rules under this strategy
...
## ====== 4b) Plot rule count variation over sample size (none vs ema) ======
...
# Calculate a unified y-axis range for comparability
...
if (!is.null(rm_none)) {
  ...
  plot_rule_traj_base(
    ...
    main_text = expression(
      alpha[add] == 0.5 ~ "," ~
        tau[merge] == 0.9),
    file_path = file.path("EEFFS_agriculture.png"),
    ylim_range = y_range
  )
}

# Plot for: EMA pruning
if (!is.null(rm_ema)) {
  x2 <- seq_along(rm_ema)
  plot_rule_traj_base(
    x = x2, y = rm_ema,
    main_text = expression(
      alpha[add] == 0.5 ~ "," ~
        ...
        tau[EMA] == 0.02 ~ "," ~
        beta == 0.95),
    file_path = file.path("EFFS_agriculture.png"),
    ylim_range = y_range
  )
}

## ====== 5) Baselines (Unified Split & Scale) ======
safe <- function(expr) tryCatch(expr, error=function(e) e)

# 5.1 Interaction-PLS (Beyaztas & Shang)
# Concat multi-channel X horizontally into a "single function" for pls_fun
run_pls_interaction <- function() {
  ...
  # ---- Concatenate multiple channels into one predictor (consistent with author's demo) ----
  ...
  # ---- Basis Safety Valve: Spline basis count cannot exceed grid length ----
...
# 5.2 fAPLS (Zhiyang Zhou): Concat multi-channel X horizontally into a "single function"
run_fapls <- function() {
  ...
  Xc <- input   # Already a 5*48 concatenation
  Yc <- output
...
# 5.3 Smooth LASSO (Centofanti et al.) via refund/fda approach
run_slasso <- function() {
  ...
  Y <- t(output)   # t_grid x n   (Note: this is in log space)
  ...
  # Cross-validation to select lambda (using official slasso.fr_cv)
  ...
  # Predict in log space: y_hat(t) = alpha(t) + ∫ x(s) * beta(s,t) ds
...
## ====== 7) Summary & Saving ======
# --- Align all result table column names and order ---
...
csv_name  <- paste0("FoF_agriculture_compare_run_", train_test_split,"_m",merge.thr,"_a",add.thr, "_t",timestamp, ".csv")
