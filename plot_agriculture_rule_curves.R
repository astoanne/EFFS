...
resolve_candidate_dirs <- function() {
  ...
  known_script_paths <- c(
    file.path(cwd, "plot_agriculture_rule_curves.R"),
    file.path(cwd, "organized_code2", "plot_agriculture_rule_curves.R"),
    file.path(cwd, "EFFS", "organized_code2", "plot_agriculture_rule_curves.R"),
...
plot_rule_snapshot <- function(center_coefs, cons_coefs, input_fd_template,
  ...
  colors <- c("black", "#f1a3b5", "#33a02c", "#7fc9ff", "#fdb462", "#8da0cb")
  ltys <- c(1, 2, 3, 4, 5, 6)
  ...
  outer_title <- sprintf("EEFFS agriculture: %s", snapshot_label)
  if (!is.na(step_label)) {
...
plot_rule_snapshot_compact <- function(center_coefs, cons_coefs, input_fd_template,
  ...
  ltys <- rep(ltys, length.out = n_rules)
  lwds <- rep(lwds, length.out = n_rules)
  center_ticks <- c(0.2, 0.4, 0.6)
  center_ylim <- c(
    min(min(centers, na.rm = TRUE), center_ticks[1]) - 0.02,
    max(max(centers, na.rm = TRUE), center_ticks[length(center_ticks)]) + 0.01
  )
  ...
  matplot(input_grid, centers, type = "l",
          ...
          ylim = center_ylim,
          yaxt = "n")
  axis(2, at = center_ticks, labels = formatC(center_ticks, format = "f", digits = 1), las = 0)
  box()
  legend("topright", legend = center_legend_expr(n_rules),
         col = colors, lty = ltys, lwd = lwds, bty = "o", cex = 0.78)
  ...
  mtext("EEFFS agriculture: peak snapshot", outer = TRUE, cex = 0.95, font = 2, line = 0.2)
}
...
message("Running agriculture EEFFS visualization: ", input_name, " -> ", output_name)
...
final_file <- file.path(output_dir, "agriculture_rule_curves_final.png")
plot_rule_snapshot(
  ...
  snapshot_label = "final rule base"
)

max_file <- file.path(output_dir, "agriculture_rule_curves_max_rules.png")
plot_rule_snapshot(
  ...
  snapshot_label = sprintf(
    "peak snapshot (%d rules)",
    result$n.clu.max.snapshot
  ),
  step_label = result$max.rule.time
)

max_compact_file <- file.path(output_dir, "agriculture_rule_curves_max_rules_compact.png")
...
metadata_file <- file.path(output_dir, "agriculture_rule_curve_plots.csv")
write.csv(
  data.frame(
